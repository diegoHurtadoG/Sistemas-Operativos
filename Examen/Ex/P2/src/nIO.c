...
static void SigioHandler(void); /* Handler de interrupciones de E/S */
...
void IOInit(void) {
  maxsize_pending=20;
  pending_tasks= malloc(maxsize_pending * sizeof(nTask));
...
void nSetNonBlockingStdio(void) {
#ifdef REOPENSTDIO
  char *ttyname();
...
int nRead(int fd, char *buf, int nbyte) {  
  ...
  START_CRITICAL();
    ...
    while (rc<0 && errno==EAGAIN) { /* No hay nada disponible */
...
int nWrite(int fd, char *buf, int nbyte) {  
  ...
  START_CRITICAL();
    ...
    while (rc<0 && errno==EAGAIN) { /* El buffer esta lleno */
      AddWaitingTask(fd, current_task);
